...
class CalibratedCamera
{
  public:
    ...
    void calibrateFromFolder(std::string path, int board_w = 6, int board_h = 9)
    {
        ...
        cout << "Buone immagini trovate:" << goodImages << " su " << fn.size() << endl;
    ...
    bool undistort(cv::Mat inputImage, cv::Mat &outputImage)
    {
        if (!calibrated)
        {
            cout << "The camera is not calibrated, either calibrate from a settings file or from a folder of images" << endl;
            return false;
        }
        ...
        cv::remap(inputImage, outputImage, map1, map2, cv::INTER_LINEAR, cv::BORDER_CONSTANT, cv::Scalar());
